# Goalset Version: 3.0


**2 Donation Wishes** | Progression level: ['cloak'] | Other tags: ['quest']

**3 Donation Wishes** | Progression level: ['cloak'] | Other tags: ['quest', 'expensive']

**2 Hunt Wishes** | Progression level: ['cloak'] | Other tags: ['quest', 'blocking']

**4 Hunt Wishes** | Progression level: ['cloak'] | Other tags: ['quest', 'blocking']

**2 Wayfarer Wishes** | Progression level: ['widow'] | Other tags: ['quest', 'flea', 'blocking']

**4 Wayfarer Wishes** | Progression level: ['act2'] | Other tags: ['quest', 'flea', 'blocking']

**2 Gather Wishes** | Progression level: ['walljump'] | Other tags: ['quest', 'blocking']

**3 Gather Wishes** | Progression level: ['widow'] | Other tags: ['quest', 'blocking']

**Complete 5 Wishes** | Progression level: ['cloak'] | Other tags: ['quest']

**Complete 7 Wishes** | Progression level: ['walljump'] | Other tags: ['quest']

**Break a Courier Item** | Progression level: ['act2'] | Other tags: ['quest']

**Crawbug Clearing** | Progression level: ['widow'] | Other tags: ['quest']

**Volatile Flintbeetles** | Progression level: ['cloak'] | Other tags: ['quest', 'locket']

**Have 3 Red Tools** | Progression level: ['dash'] | Other tags: ['tool', 'craft', 'blocking']

**Have 5 Red Tools** | Progression level: ['cloak'] | Other tags: ['tool', 'craft', 'blocking']

**Have 3 Blue Tools** | Progression level: ['dash'] | Other tags: ['tool', 'blocking']

**Have 3 Yellow Tools** | Progression level: ['early'] | Other tags: ['tool', 'blocking']

**Have 5 Yellow Tools** | Progression level: ['cloak'] | Other tags: ['tool', 'blocking']

**Have 2 of Each Tool Type** | Progression level: ['dash'] | Other tags: ['tool', 'blocking']

**Check Straight, Threefold, and Long Pin** | Progression level: ['cloak'] | Other tags: ['tool']

**Dead Bug's Purse & a Silkeater** | Progression level: ['walljump'] | Other tags: ['tool']

**Barbed Bracelet & Fractured Mask** | Progression level: ['walljump'] | Other tags: ['tool']

**Magnetite Dice & Magnetite Brooch** | Progression level: ['walljump'] | Other tags: ['tool']

**Shard Pendant & Magnetite Brooch** | Progression level: ['early'] | Other tags: ['tool']

**Warding Bell & Claw Mirror** | Progression level: ['act2'] | Other tags: ['tool']

**Warding Bell & Sawtooth Circlet** | Progression level: ['clawline'] | Other tags: ['tool', 'craft']

**Magma Bell & Curveclaw** | Progression level: ['early'] | Other tags: ['tool', 'craft']

**Weighted Belt & Ascendant's Grip** | Progression level: ['act2'] | Other tags: ['tool', 'expensive']

**Scuttlebrace & Silkspeed** | Progression level: ['clawline'] | Other tags: ['tool', 'craft']

**Pimpillo & Voltvessels** | Progression level: ['faydown'] | Other tags: ['tool', 'craft']

**Delver's Drill** | Progression level: ['act2'] | Other tags: ['tool']

**Pollip Pouch** | Progression level: ['walljump'] | Other tags: ['tool', 'quest']

**Pollip Pouch & Sting Shard** | Progression level: ['walljump'] | Other tags: ['tool', 'craft', 'quest']

**Tacks** | Progression level: ['walljump'] | Other tags: ['tool', 'quest']

**Quick Sling** | Progression level: ['faydown'] | Other tags: ['tool']

**Weavelight & Injector Band** | Progression level: ['act2'] | Other tags: ['tool', 'fight']

**Plasmium Phial** | Progression level: ['walljump'] | Other tags: ['tool', 'quest', 'key']

**Memory Crystal** | Progression level: ['faydown'] | Other tags: ['tool']

**Flintslate** | Progression level: ['walljump'] | Other tags: ['tool', 'key']

**Silkshot** | Progression level: ['widow'] | Other tags: ['tool', 'craft']

**Multibinder** | Progression level: ['act2'] | Other tags: ['tool', 'quest', 'expensive']

**Rosary Cannon** | Progression level: ['clawline'] | Other tags: ['tool', 'key']

**Cogfly** | Progression level: ['clawline'] | Other tags: ['tool', 'craft']

**Snare Setter** | Progression level: ['faydown'] | Other tags: ['tool']

**Get or Check Conchcutter** | Progression level: ['clawline'] | Other tags: ['tool', 'fight']

**Pale Lake Craftmetal** | Progression level: ['faydown'] | Other tags: ['location', 'collection']

**Upgrade Druid's Eye** | Progression level: ['faydown'] | Other tags: ['collection', 'scattered', 'tool']

**Sell a Mossberry** | Progression level: ['early'] | Other tags: ['collection']

**Wanderer's Crest** | Progression level: ['walljump'] | Other tags: ['upgrade', 'hardsave']

**Reaper's Crest** | Progression level: ['cloak'] | Other tags: ['upgrade', 'hardsave']

**Beast's Crest** | Progression level: ['cloak'] | Other tags: ['upgrade', 'hardsave', 'hard']

**Architect's Crest** | Progression level: ['faydown'] | Other tags: ['upgrade', 'hardsave', 'hard', 'tool', 'collection']

**Needle Strike** | Progression level: ['walljump'] | Other tags: ['upgrade', 'fight']

**Upgrade Needle Twice** | Progression level: ['act2'] | Other tags: ['upgrade', 'quest']

**Thread Storm** | Progression level: ['cloak'] | Other tags: ['upgrade', 'hardsave']

**Sharpdart** | Progression level: ['cloak'] | Other tags: ['upgrade', 'key', 'hardsave']

**Evolved Hunter's Crest** | Progression level: ['widow'] | Other tags: ['upgrade', 'hardsave']

**Unlock Vesticrest** | Progression level: ['act2'] | Other tags: ['upgrade', 'hardsave', 'collection', 'scattered']

**Faydown Cloak** | Progression level: ['faydown'] | Other tags: ['upgrade', 'hardsave', 'blocking']

**Apostate Key** | Progression level: ['faydown'] | Other tags: ['collection']

**Open Deep Docks Key Door** | Progression level: ['early'] | Other tags: ['key', 'location', 'fight']

**3 Silk Skills** | Progression level: ['widow'] | Other tags: ['upgrade', 'hardsave']

**4 Silk Skills** | Progression level: ['act2'] | Other tags: ['upgrade', 'hardsave']

**Check Farfields Escape Mask Shard** | Progression level: ['clawline'] | Other tags: ['upgrade', 'hard']

**Check Slab Mask Shard** | Progression level: ['faydown'] | Other tags: ['upgrade']

**Check Bilewater Mask Shard** | Progression level: ['faydown'] | Other tags: ['upgrade', 'hard']

**Check Deep Docks Mask Shard** | Progression level: ['walljump'] | Other tags: ['upgrade']

**Check Shellwood + Vaults Mask Shards** | Progression level: ['act2'] | Other tags: ['upgrade']

**Check Blasted Steps Mask Shard** | Progression level: ['faydown'] | Other tags: ['upgrade']

**Check Cogwork Core Mask Shard** | Progression level: ['act2'] | Other tags: ['upgrade']

**Check Wisp Thicket Mask Shard** | Progression level: ['faydown'] | Other tags: ['upgrade']

**Check Mt. Fay Mask Shard** | Progression level: ['faydown'] | Other tags: ['upgrade']

**Craggler** | Progression level: ['early'] | Other tags: ['fight']

**Moorwing** | Progression level: ['cloak'] | Other tags: ['fight']

**Savage Beastfly** | Progression level: ['cloak'] | Other tags: ['fight', 'hard']

**Great Conchflies (Blasted Steps)** | Progression level: ['walljump'] | Other tags: ['fight', 'blocking']

**Raging Conchfly (Sands of Karak)** | Progression level: ['clawline'] | Other tags: ['fight']

**Moss Mother Duo** | Progression level: ['widow'] | Other tags: ['fight']

**Defeat a Rhinogrund** | Progression level: ['walljump'] | Other tags: ['fight', 'hard']

**Disable the Clawmaidens** | Progression level: ['faydown'] | Other tags: ['fight']

**Fourth Chorus** | Progression level: ['cloak'] | Other tags: ['fight']

**Cogwork Dancers** | Progression level: ['act2'] | Other tags: ['fight', 'blocking']

**Last Judge** | Progression level: ['widow'] | Other tags: ['fight']

**Phantom** | Progression level: ['widow'] | Other tags: ['fight']

**Trobbio** | Progression level: ['act2'] | Other tags: ['fight', 'tool', 'blocking']

**Disgraced Chef Lugoli** | Progression level: ['faydown'] | Other tags: ['fight', 'hard']

**Father of the Flame** | Progression level: ['faydown'] | Other tags: ['fight', 'tool', 'hard']

**Forebrothers Signis & Gron** | Progression level: ['clawline'] | Other tags: ['fight', 'hard', 'blocking']

**Defeat a Covetous Pilgrim** | Progression level: ['cloak'] | Other tags: ['fight']

**Cogwork Clapper Journal Entry** | Progression level: ['act2'] | Other tags: ['fight']

**Sister Splinter** | Progression level: ['cloak'] | Other tags: ['fight']

**Voltvyrm** | Progression level: ['faydown'] | Other tags: ['fight', 'tool']

**Skull Tyrant** | Progression level: ['walljump'] | Other tags: ['fight', 'quest']

**Free Green Prince** | Progression level: ['walljump'] | Other tags: ['npc', 'key']

**Win a Loddie Minigame** | Progression level: ['widow'] | Other tags: ['npc']

**Talk to Styx & Huntress** | Progression level: ['faydown'] | Other tags: ['npc', 'hardsave']

**Wreath of Purity** | Progression level: ['faydown'] | Other tags: ['tool']

**Break Both of Styx's One-ways** | Progression level: ['widow'] | Other tags: ['location']

**Talk to Loam** | Progression level: ['act2'] | Other tags: ['npc']

**Talk to Mask Maker** | Progression level: ['faydown'] | Other tags: ['npc']

**Buy From Grindle (No String)** | Progression level: ['faydown'] | Other tags: ['npc']

**Check Blasted Steps Silkeater & Craftmetal** | Progression level: ['walljump'] | Other tags: ['collection']

**Pay To Confess** | Progression level: ['act2'] | Other tags: ['location']

**Choral Chambers Ventrica** | Progression level: ['act2'] | Other tags: ['location']

**High Halls Ventrica** | Progression level: ['clawline'] | Other tags: ['location']

**Memorium Ventrica** | Progression level: ['faydown'] | Other tags: ['location']

**Obtain 3 Craftmetal** | Progression level: ['act2'] | Other tags: ['collection', 'scattered', 'craft']

**Obtain 5 Craftmetal** | Progression level: ['clawline'] | Other tags: ['collection', 'scattered', 'craft']

**Crustnut** | Progression level: ['clawline'] | Other tags: ['collection']

**Mossberry Soup** | Progression level: ['act2'] | Other tags: ['collection']

**Vintage Nectar** | Progression level: ['act2'] | Other tags: ['npc', 'collection', 'expensive']

**Twisted Bud** | Progression level: ['clawline'] | Other tags: ['collection']

**Use 2 Simple Keys** | Progression level: ['clawline'] | Other tags: ['collection', 'scattered']

**1 Extra Mask** | Progression level: ['widow'] | Other tags: ['upgrade']

**2 Extra Masks** | Progression level: ['clawline'] | Other tags: ['upgrade']

**Pay for a Flea Spa** | Progression level: ['walljump'] | Other tags: ['npc', 'flea']

**Check Greymoor Fleas [2]** | Progression level: ['walljump'] | Other tags: ['npc', 'flea']

**Check Far Fields Fleas [2]** | Progression level: ['walljump'] | Other tags: ['flea']

**Check Shellwood & Bellhart Fleas [2]** | Progression level: ['walljump'] | Other tags: ['flea']

**Check Deep Docks Fleas [3]** | Progression level: ['clawline'] | Other tags: ['flea', 'hard']

**Check Wormways & Blasted Steps Fleas [2]** | Progression level: ['walljump'] | Other tags: ['flea']

**Check Underworks Fleas [2]** | Progression level: ['faydown'] | Other tags: ['flea']

**Check Lower Bilewater & Hunter's March Fleas [2]** | Progression level: ['walljump'] | Other tags: ['flea']

**Free 8 Fleas** | Progression level: ['cloak'] | Other tags: ['flea']

**Free 10 Fleas** | Progression level: ['widow'] | Other tags: ['flea']

**Free 12 Fleas** | Progression level: ['act2'] | Other tags: ['flea']

**Free 14 Fleas** | Progression level: ['clawline'] | Other tags: ['flea']

**Meet Caravan at the Grand Gate [12]** | Progression level: ['act2'] | Other tags: ['flea']

**Check Sinner's Road & Vaults Fleas [2]** | Progression level: ['clawline'] | Other tags: ['flea']

**Check Sands of Karak Flea [1]** | Progression level: ['clawline'] | Other tags: ['flea']

**Check Exhaust Organ Flea [1]** | Progression level: ['widow'] | Other tags: ['flea']

**Check Three Citadel Fleas** | Progression level: ['act2'] | Other tags: ['flea']

**Check Mt. Fay Flea [1]** | Progression level: ['clawline'] | Other tags: ['flea']

**Beat Big Flea** | Progression level: ['faydown'] | Other tags: ['fight']

**Talk to Vog Anywhere** | Progression level: ['faydown'] | Other tags: ['flea', 'npc']

**Check Deep Docks Spool Fragments (2)** | Progression level: ['clawline'] | Other tags: ['upgrade']

**Check Greymoor Spool Fragment** | Progression level: ['walljump'] | Other tags: ['upgrade']

**Check Grand Gate Spool Fragment** | Progression level: ['act2'] | Other tags: ['upgrade']

**Check High Halls Spool Fragment** | Progression level: ['faydown'] | Other tags: ['upgrade']

**1 Spool Upgrade** | Progression level: ['early'] | Other tags: ['upgrade']

**2 Spool Upgrades** | Progression level: ['walljump'] | Other tags: ['upgrade']

**3 Spool Upgrades** | Progression level: ['clawline'] | Other tags: ['upgrade']

**Have 3 Rosary Necklaces (no purchasing)** | Progression level: ['act2'] | Other tags: ['collection', 'scattered']

**Have 6 Rosary Strings (no purchasing)** | Progression level: ['widow'] | Other tags: ['collection', 'scattered']

**Break 8 Rosary Strings (no purchasing)** | Progression level: ['act2'] | Other tags: ['collection', 'scattered']

**Break 3 Shard Bundles (no purchasing)** | Progression level: ['walljump'] | Other tags: ['collection', 'scattered']

**Break 5 Shard Bundles (no purchasing)** | Progression level: ['clawline'] | Other tags: ['collection', 'scattered']

**Obtain 3 Silkeaters** | Progression level: ['walljump'] | Other tags: ['collection', 'scattered']

**Obtain 5 Silkeaters** | Progression level: ['act2'] | Other tags: ['collection', 'scattered']

**2 Tool Pouch Upgrades** | Progression level: ['cloak'] | Other tags: ['collection', 'scattered']

**2 Crafting Kits** | Progression level: ['dash'] | Other tags: ['collection', 'scattered']

**3 Tool Pouch Upgrades/Crafting Kits** | Progression level: ['widow'] | Other tags: ['collection', 'scattered']

**Obtain 5 Memory Lockets** | Progression level: ['walljump'] | Other tags: ['collection', 'scattered', 'locket']

**Fully Unlock a Non-Hunter's Crest** | Progression level: ['walljump'] | Other tags: ['collection', 'scattered', 'locket']

**Have 3 Different Types of Relic** | Progression level: ['cloak'] | Other tags: ['collection', 'scattered', 'relic']

**Obtain 4 Different Types of Relic** | Progression level: ['widow'] | Other tags: ['collection', 'scattered', 'relic']

**Obtain 2 Rune Harps** | Progression level: ['widow'] | Other tags: ['collection', 'relic']

**Obtain 3 Bone Scrolls** | Progression level: ['clawline'] | Other tags: ['collection', 'relic', 'scattered']

**Talk to Shakra at 5 Locations** | Progression level: ['widow'] | Other tags: ['npc', 'missable']

**Fix the Sinner's Road Bench** | Progression level: ['walljump'] | Other tags: ['location']

**Talk to Shakra in Bilewater** | Progression level: ['faydown'] | Other tags: ['npc', 'location']

**Talk to Shakra in Sands of Karak** | Progression level: ['clawline'] | Other tags: ['npc', 'location']

**Buy 5 Items from Shakra** | Progression level: ['widow'] | Other tags: ['npc', 'location']

**Buy 3 Items from Frey (no bundle/string)** | Progression level: ['widow'] | Other tags: ['npc', 'location']

**Talk to Huntress** | Progression level: ['faydown'] | Other tags: ['npc']

**Save Jubilana** | Progression level: ['act2'] | Other tags: ['npc', 'quest']

**Squirrm Journal Entry** | Progression level: ['widow'] | Other tags: ['collection']

**Skullwing Journal Entry** | Progression level: ['widow'] | Other tags: ['collection']

**Cragglite Journal Entry** | Progression level: ['act2'] | Other tags: ['collection']

**Gromling Journal Entry** | Progression level: ['cloak'] | Other tags: ['collection', 'key']

**Mite Mother Journal Entry** | Progression level: ['cloak'] | Other tags: ['collection']

**Massive Mossgrub Journal Entry** | Progression level: ['faydown'] | Other tags: ['collection']

**Deep Diver Journal Entry** | Progression level: ['dash'] | Other tags: ['collection']

**Obtain 2 Beast Shards** | Progression level: ['dash'] | Other tags: ['fight']

**Obtain 3 Beast Shards** | Progression level: ['widow'] | Other tags: ['fight']

**Obtain 4 Beast Shards** | Progression level: ['act2'] | Other tags: ['fight']

**Bilewater Bellway** | Progression level: ['walljump'] | Other tags: ['location']

**Talk to Forge Daughter and Twelfth Architect** | Progression level: ['clawline'] | Other tags: ['npc', 'blocking']

**1 Cogheart Piece** | Progression level: ['act2'] | Other tags: ['collection']

**2 Cogheart Pieces** | Progression level: ['act2'] | Other tags: ['collection']

**Activate Second Sentinel** | Progression level: ['faydown'] | Other tags: ['collection', 'npc']

**Meet Sherma in the Spa** | Progression level: ['act2'] | Other tags: ['npc']

**2 Silk Hearts** | Progression level: ['faydown'] | Other tags: ['upgrade', 'fight', 'hardsave', 'hard']

**Unravelled Journal Entry** | Progression level: ['faydown'] | Other tags: ['fight', 'hard']

**Architect's Melody** | Progression level: ['clawline'] | Other tags: ['location', 'melody']

**Conductor's Melody** | Progression level: ['clawline'] | Other tags: ['fight', 'melody', 'hard']

**Vaultkeeper's Melody** | Progression level: ['act2'] | Other tags: ['fight', 'melody']

**Witch Crest** | Progression level: ['clawline'] | Other tags: ['hardsave']

**Beastling Call** | Progression level: ['faydown'] | Other tags: ['fight', 'blocking']

**3 Non-Hunter Crests** | Progression level: ['act2'] | Other tags: ['upgrade', 'hardsave']

**Pin Badge** | Progression level: ['clawline'] | Other tags: ['fight']

**Inspect Magnetite in Brightvein** | Progression level: ['faydown'] | Other tags: ['collection', 'location']

**Clear First Coral Tower Arena** | Progression level: ['faydown'] | Other tags: ['fight', 'hard']

**Visit Styx's Mask Room** | Progression level: ['faydown'] | Other tags: ['location', 'npc']

**Watcher at the Edge** | Progression level: ['faydown'] | Other tags: ['fight', 'hard']

**Check Brightvein Mask Shard** | Progression level: ['faydown'] | Other tags: ['upgrade']

**Win A Race** | Progression level: ['faydown'] | Other tags: ['location', 'npc']

**Shaman Crest** | Progression level: ['faydown'] | Other tags: ['upgrade']

**Have 5 Blue Tools** | Progression level: ['walljump'] | Other tags: ['tool', 'craft', 'blocking']

**Have 4 of Each Tool Type** | Progression level: ['cloak'] | Other tags: ['tool', 'craft', 'blocking']

**Crawfather** | Progression level: ['act2'] | Other tags: ['fight']

**Claw Mirrors** | Progression level: ['faydown'] | Other tags: ['tool', 'upgrade']

**Longclaw** | Progression level: ['faydown'] | Other tags: ['tool']

**Wispfire Lantern** | Progression level: ['faydown'] | Other tags: ['tool']

**Egg of Flealia** | Progression level: ['faydown'] | Other tags: ['tool', 'flea']

**Spider Strings** | Progression level: ['faydown'] | Other tags: ['tool']

**Buy Out Pebb's Shop** | Progression level: ['widow'] | Other tags: ['tool']

**Snitchfly Journal Entry** | Progression level: ['act2'] | Other tags: ['collection']

**Dreg Husk Journal Entry** | Progression level: [] | Other tags: ['collection']

**Check Bilewater Memory Locket** | Progression level: ['faydown'] | Other tags: ['locket']

**Break 5 Rosary Necklaces** | Progression level: ['act2'] | Other tags: ['collection', 'scattered']

**4 Tool Pouch Upgrades/Crafting Kits** | Progression level: ['widow'] | Other tags: ['collection', 'scattered']

**Beat First Marrow Arena** | Progression level: ['early'] | Other tags: ['fight']

**Karmelita Memory Locket** | Progression level: ['faydown'] | Other tags: ['locket']

**2 Psalm Cylinders** | Progression level: ['act2'] | Other tags: ['collection', 'relic', 'scattered']

**Rune Rage** | Progression level: ['faydown'] | Other tags: ['upgrade', 'hardsave', 'fight', 'hard']

**Check Pristine Core** | Progression level: ['faydown'] | Other tags: ['fight']

**Obtain Hornet Statue** | Progression level: ['act2'] | Other tags: ['quest', 'expensive']

**Inspect Flintstone in Deep Docks** | Progression level: ['clawline'] | Other tags: ['collection', 'location']

**Inspect Voltridian in Voltnest** | Progression level: ['faydown'] | Other tags: ['collection', 'location']

**Curvesickle** | Progression level: ['faydown'] | Other tags: ['tool', 'upgrade']

**Snitch Pick & Thief's Mark** | Progression level: ['faydown'] | Other tags: ['tool']

**4 Donation Wishes** | Progression level: ['cloak'] | Other tags: ['quest', 'expensive']

**5 Donation Wishes** | Progression level: ['cloak'] | Other tags: ['quest', 'expensive']

**Use Architect's Key** | Progression level: ['faydown'] | Other tags: ['tool', 'collection', 'scattered']

**Talk to Grindle & Kratt** | Progression level: ['faydown'] | Other tags: ['npc']

**Hit Groal the Great** | Progression level: ['faydown'] | Other tags: ['fight']

**Get or Check Throwing Ring** | Progression level: ['faydown'] | Other tags: ['collection', 'scattered', 'tool']

**Shrine Guardian Seth** | Progression level: ['faydown'] | Other tags: ['collection', 'scattered', 'tool']

